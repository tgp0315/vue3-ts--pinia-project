    ...
    return new Promise((resolve) => {
      ...
      reader.onload = () => {
        ...
        img.onload = () => {
          ...
          const newImgData: string = canvas.toDataURL(fileType, 0.9)

          // 压缩宽高后的图像大小
          const newImgSize: number = this.fileSizeKB(newImgData)

          if (newImgSize > this.fileSize) {
            console.log('图片尺寸太大!' + fileSize + ' >> ' + newImgSize)
          }

          const blob: Blob = this.dataURLtoBlob(newImgData, fileType)
          const nfile: File = new File([blob], this.file.name)
          resolve(nfile)
        }
      }
    })
